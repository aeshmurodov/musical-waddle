                                        ...
                                        <div key={index} className="col-12">
                                            <div className="blog-post-box">
                                                <div className="blog-img">
                                                    <Link className="blog-img-link" href={`light/${item.slug}`}>
                                                        <Image src={item.mainImage} alt={item.title} placeholder="blur" />
                                                    </Link>
                                                    <div className="blog-category">
                                                        <Link className="button button-sm button-style-2 button-font-2" href={item.categories[0].url}>{item.categories[0].name}</Link>
                                                    </div>
                                                </div>
                                                <div className="pt-4">
                                                    <ul className="list-inline-dot sm-heading mb-2">
                                                        <li><Link className="link-hover" href={`light/${item.slug}`}>by {item.postedBy}</Link></li>
                                                        <li><Link className="link-hover" href={`light/${item.slug}`}>{item.date}</Link></li>
                                                    </ul>
                                                    <h2>
                                                        <Link className="link-hover-2" href={`light/${item.slug}`}>
                                                            {item.title}
                                                        </Link>
                                                    </h2>
                            ...
                            <div className="col-12 col-lg-5 col-xl-4">
                                ...
                                <div className="fancy-box">
                                    ...
                                    <ul className="list-unstyled mt-3">
                                        {blogData.blogListSidebarDark.categories.map((item, index) => (
                                            <li key={index}><Link className="link-hover" href={item.url}>{item.name}</Link><span className="ps-2">- {item.countOfPosts}</span></li>
                                        ))}
                                    </ul>
                                </div>
                                {/* Trending Posts */}
                                <div className="fancy-box mt-4">
                                    ...
                                    {blogData.blogListSidebarDark.posts.filter(item => item.isTrending).map((item, index) => (
                                        <div key={index} className="sidebar-post mt-3">
                                            ...
                                            <div className="post-title">
                                                <h6 className="mb-0"><Link href={`light/${item.slug}`}>{item.title}</Link></h6>
                                                <span className="font-small">Posted on {item.date}</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="fancy-box mt-4">
                                    ...
                                    <ul className="list-inline-lg mt-3">
                                        {blogData.blogListSidebarDark.tags.map((item, index) => (
                                            <li key={index}><Link className="link-hover" href={item.url}>#{item.name}</Link></li>
                                        ))}
                                    </ul>
                                </div>
                                {/* Archive */}
