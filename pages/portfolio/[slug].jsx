                        ...
                        <div className="row g-4 mt-4">
                            ...
                            <div className="col-12 col-lg-6 col-xl-3">
                                <div className="fancy-box">
                                    ...
                                    <Link className="link-hover" href={project.projectLink.url}>{project.projectLink.title}</Link>
                                </div>
                            </div>
                        </div> {/* end row  */}
                    </div> {/* end container  */}
...
export async function getStaticProps({ params }) {
    ...
    const agencyProjects = portfolioData.agency.projects.find((project) => project.slug === slug);
    // Search for the Portfolio project with the matching slug in the business section
    const businessProjects = portfolioData.business.projects.find((project) => project.slug === slug);
    // Search for the Portfolio project with the matching slug in the portfolio section
    const portfolioProjects = portfolioData.portfolio.projects.find((project) => project.slug === slug);
    // Search for the Portfolio project with the matching slug in the startup section
    const startupProjects = portfolioData.startup.projects.find((project) => project.slug === slug);
    // Search for the Portfolio project with the matching slug in the marketing section
    const marketingProjects = portfolioData.marketing.projects.find((project) => project.slug === slug);

    // Check if the project is found in either section
    if (!agencyProjects && !businessProjects && !portfolioProjects && !startupProjects && !marketingProjects) {
        return {
            notFound: true,
        };
    }
    ...
    const project = agencyProjects || businessProjects || portfolioProjects || startupProjects || marketingProjects;
